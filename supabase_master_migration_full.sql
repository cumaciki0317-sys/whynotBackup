...
COMMENT ON COLUMN public.ideas.pdf_attachment_path IS
  'Private Supabase Storage path for a PDF, PNG, or JPEG idea reference file';
COMMENT ON COLUMN public.ideas.pdf_attachment_name IS
  'Original reference file display name; access is authorized by the application server';
COMMENT ON COLUMN public.ideas.pdf_attachment_size IS
  'Reference file size in bytes; max 10 MiB';
